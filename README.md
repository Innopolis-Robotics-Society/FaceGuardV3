...
- Enough local storage for the PostgreSQL volume and InsightFace models. The first model initialization may need internet access to populate `docker/insightface_models/buffalo_s`; normal operation is local after the assets are present.
...
- **Model startup failure:** verify `docker/insightface_models/minifasnet.onnx`, the configured `LIVENESS_MODEL_PATH`, storage permissions, and the initially populated `buffalo_s` model directory.
