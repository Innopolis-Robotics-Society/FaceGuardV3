...
Administrators must ensure that the `.env` file is correctly populated. Furthermore, the `insightface_models` directory must be mounted to cache the ONNX models, and the `pgdata` volume must be persisted to avoid data loss on container restarts.
