...
- Initial access to model assets. The tracked MiniFASNet model is mounted from `docker/insightface_models`; InsightFace may download `buffalo_s` on first initialization if it is absent.
