...
|---|---|---:|---:|---|
| `backend/main.py` | Recognition/enrollment orchestration, operation lock, WebSocket/frame contract, lifecycle | 30% | 82.43% | Pass |
| `backend/camera.py` | One capture owner, latest-frame replacement, release/error paths | 30% | 89.24% | Pass |
...
| `backend/faceguard/recognize.py` | InsightFace/liveness adapter, embeddings, thresholds | 30% | 99.20% | Pass |
| `backend/core/security.py` | JWT signing, expiry/signature/subject validation | 30% | 100.00% | Pass |
| `backend/leds.py` | Color mapping, stale-worker exclusion, chip selection, cleanup | 30% | 79.12% | Pass |

The post-merge audit run covered 1,638 backend statements at 87.24% and passed 164 tests. The target branch's model bootstrap and native-thread runtime setup are also covered at 88.89% each, but they are deployment support rather than separate access-decision gates. The policy is enforced from `reports/coverage.json` by `scripts/check_critical_coverage.py`; a high global percentage cannot hide a critical module below 30%.

## Test suites

| Suite | Behavior/risk focus | Latest local result |
|---|---|---:|
| `tests/unit` | Recognition math/orchestration, liveness integration, camera ownership/latest-frame/release, model bootstrap, JWT/WS auth, bbox metadata, mutual exclusion, database rollback/timestamps, duplicate comparison, GPIO generations/cleanup | 146 passed |
...
| Frontend Vitest | Runtime URLs/subprotocol, bbox projection/preview, browser/backend modes, page lifecycle, one browser frame in flight and exact response JPEG | 18 passed |
...
The 2026-07-17 post-merge audit built the ordinary backend, Docker Hub backend, and frontend Dockerfiles. Both backend images successfully imported the pinned `opencv-python-headless` 4.11.0 and InsightFace 1.0.1 runtimes. The ordinary image then started against the isolated PostgreSQL service and persisted model cache and returned `/health` with `models_ready: true`; the frontend preview image served HTTP. InsightFace 1.0.1's upstream wheel metadata names the GUI `opencv-python` distribution, so `pip check` reports that metadata requirement even though the deliberately installed headless `cv2` runtime imports successfully; installing both overlapping OpenCV distributions is intentionally avoided. These are local container checks, not ordinary PR CI or physical-camera/model-accuracy evidence; the reproducible build commands are in the root README.
