...
export default function CameraPreview({
  ...
  const [containerSize, setContainerSize] = useState<Dimensions>(emptyDimensions);
  const [mediaSize, setMediaSize] = useState<Dimensions>(emptyDimensions);
  ...
  const projection = useMemo(() => {
    const sourceWidth = frameWidth && frameWidth > 0 ? frameWidth : mediaSize.width;
    const sourceHeight = frameHeight && frameHeight > 0 ? frameHeight : mediaSize.height;
    if (!box) return null;
    return projectBBox({
      box,
    ...
    <div className="camera-container" ref={containerRef}>
      {remoteFrame ? (
        ...
        <video
          ...
          onResize={updateVideoDimensions}
        />
      ) : (
